...
export const socOperationsLabs: LabDefinition[] = [
  // BEGINNER LABS
  {
    ...
    steps: [
      { number: 1, title: "Detect Issue", description: "Scan for CloudTrail problems.", hint: "Type 'scan' to identify issues.", completionFeedback: "You identified the CloudTrail gap. Without logging, attackers can operate undetected - every API call goes unrecorded, making incident investigation impossible." },
      { number: 2, title: "Check Trail Status", description: "Review CloudTrail configuration.", hint: "Type 'aws cloudtrail status main-trail'.", completionFeedback: "You confirmed the trail is disabled. SOC analysts rely on CloudTrail as the primary source of truth for all AWS activity - this blind spot is critical." },
      { number: 3, title: "Enable Logging", description: "Turn CloudTrail logging back on.", hint: "Type 'aws cloudtrail enable main-trail'.", completionFeedback: "Logging restored! You've closed a major visibility gap. CloudTrail is essential for SOC operations - it captures who did what, when, and from where across your entire AWS environment." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Detect Session", description: "Scan for unusual SSM activity.", hint: "Type 'scan' to identify issues.", completionFeedback: "You detected the suspicious session. SSM provides shell access without SSH - attackers love it because it bypasses traditional network monitoring. Vigilant detection is essential." },
      { number: 2, title: "Review Sessions", description: "List all active SSM sessions.", hint: "Type 'aws ssm list-sessions'.", completionFeedback: "You reviewed active sessions and identified the threat. The session originated from a suspicious IP with an unknown user - classic indicators of unauthorized access." },
      { number: 3, title: "Terminate Session", description: "End the suspicious session.", hint: "Type 'aws ssm terminate ssm-session-xyz'.", completionFeedback: "Session terminated! You cut off the attacker's access immediately. In SOC operations, every second an unauthorized session stays active is another opportunity for data theft or lateral movement." }
    ],
    resources: [
...
export const socEngineerLabs: LabDefinition[] = [
  // BEGINNER LABS (4)
  {
    ...
    steps: [
      { number: 1, title: "Access SIEM Console", description: "Connect to the SIEM dashboard.", hint: "Type 'siem connect'.", completionFeedback: "You connected to the SIEM console. Having quick access to your central monitoring platform is the first step in any SOC engineer's workflow - this is your command center for threat detection." },
      { number: 2, title: "Review Alert Rules", description: "List current alerting rules.", hint: "Type 'siem list-rules'.", completionFeedback: "You audited the existing alert rules. Regular rule reviews help SOC engineers identify detection gaps - you can't respond to threats you're not alerting on." },
      { number: 3, title: "Create Failed Login Alert", description: "Add alerting for 5+ failed logins.", hint: "Type 'siem create-rule failed-logins'.", completionFeedback: "Alert rule created! Failed login monitoring is fundamental to detecting brute force attacks and credential stuffing. This single rule can catch attackers before they gain access." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Check Log Sources", description: "List currently integrated log sources.", hint: "Type 'siem list-sources'.", completionFeedback: "You listed all integrated log sources. SOC engineers must know exactly what visibility they have - blind spots in log collection mean blind spots in threat detection." },
      { number: 2, title: "Identify Missing Source", description: "Firewall logs are not integrated.", hint: "Type 'scan' to verify.", completionFeedback: "You identified the missing firewall logs. Firewalls are your perimeter defense - without their logs, you'd miss external attack attempts, port scans, and blocked intrusion attempts." },
      { number: 3, title: "Add Log Source", description: "Integrate firewall logs into SIEM.", hint: "Type 'siem add-source firewall-logs'.", completionFeedback: "Firewall logs integrated! Centralized logging is the backbone of SOC operations. Every new log source expands your detection surface and reduces the attacker's ability to hide." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Access Dashboard", description: "Open the SOC monitoring dashboard.", hint: "Type 'dashboard open soc-dashboard'.", completionFeedback: "You opened the SOC dashboard. Dashboards give SOC engineers at-a-glance visibility into security posture - a well-designed dashboard can reduce mean time to detect (MTTD)." },
      { number: 2, title: "Review Widgets", description: "Check existing dashboard widgets.", hint: "Type 'dashboard list-widgets'.", completionFeedback: "You reviewed the current widgets. Understanding what's already visualized helps identify monitoring gaps - effective dashboards show key metrics without overwhelming analysts." },
      { number: 3, title: "Add Threat Widget", description: "Add a real-time threat indicator widget.", hint: "Type 'dashboard add-widget threat-indicator'.", completionFeedback: "Threat indicator widget added! Real-time visualization helps SOC teams spot anomalies instantly. Good dashboards turn raw data into actionable intelligence at a glance." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Review Severity Levels", description: "Check current alert severity configuration.", hint: "Type 'siem show-severity-config'.", completionFeedback: "You reviewed the severity configuration. Proper alert classification is critical for SOC prioritization - analysts must know which alerts to investigate first." },
      { number: 2, title: "Identify Misconfiguration", description: "Critical alerts are set to low priority.", hint: "This causes alert fatigue and missed incidents.", completionFeedback: "You identified the severity misconfiguration. When critical alerts are marked low-priority, real attacks get buried in noise. This is how breaches go unnoticed for months." },
      { number: 3, title: "Fix Severity Mapping", description: "Correct the severity classification.", hint: "Type 'siem fix-severity critical-alerts'.", completionFeedback: "Severity levels corrected! Proper classification ensures critical threats get immediate attention. This directly reduces mean time to respond (MTTR) and prevents alert fatigue." }
    ],
    resources: [
