...
export const storageSecurityLabs: LabDefinition[] = [
  ...
  // INTERMEDIATE LABS (5-7 steps, multiple phases)
  {
    ...
    steps: [
      { number: 1, title: "Assess Current State", description: "Scan infrastructure for backup vulnerabilities.", hint: "Type 'scan' to identify issues.", intel: "Ransomware operators specifically target backups first to maximize impact.", completionFeedback: "You identified backup vulnerabilities. Understanding your weak points is the first step to defense." },
      { number: 2, title: "Review Versioning Status", description: "Check which buckets have versioning enabled.", hint: "Type 'aws s3 ls-versioning' to review.", intel: "S3 versioning keeps multiple variants of an object. When enabled, you can recover any previous version.", completionFeedback: "You audited versioning status. Knowing which buckets lack protection lets you prioritize remediation." },
      { number: 3, title: "Understand the Risk", description: "Without versioning, deleted files cannot be recovered. This violates backup compliance requirements.", hint: "Consider what happens if someone accidentally deletes critical backups.", intel: "NIST CSF: PR.IP-4 requires backups to be conducted, maintained, and tested.", completionFeedback: "You understand the risk. Ransomware attackers target backups first - versioning is your insurance policy." },
      { number: 4, title: "Enable Versioning", description: "Turn on versioning for the backup bucket.", hint: "Type 'aws s3 enable-versioning disaster-recovery-backup'.", intel: "Pro tip: Combine versioning with MFA Delete for an additional layer of protection against accidental or malicious deletion.", completionFeedback: "Versioning enabled! You can now recover deleted or modified files. This is essential ransomware protection." },
      { number: 5, title: "Verify Configuration", description: "Confirm versioning is now active.", hint: "Type 'aws s3 ls-versioning' to verify.", intel: "Once enabled, versioning cannot be fully disabled - only suspended. This is a feature, not a bug.", completionFeedback: "You verified the fix. Always confirm your changes took effect - trust but verify." },
      { number: 6, title: "Document Compliance", description: "Run a final scan to confirm compliance status.", hint: "Type 'scan' to generate compliance report.", intel: "Document this change for your audit trail. Compliance isn't just about fixing - it's about proving you fixed it.", completionFeedback: "You documented the remediation. Audit trails prove you fixed issues - critical for compliance reviews." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Identify Policy Risk", description: "Scan for overly permissive bucket policies.", hint: "Type 'scan' to find misconfigurations.", intel: "AWS Config rule 's3-bucket-public-read-prohibited' can automatically detect and alert on these issues.", completionFeedback: "You found the overly permissive policy. Wildcard permissions are a top cloud misconfiguration." },
      { number: 2, title: "Review Current Policy", description: "Examine the bucket's IAM policy document.", hint: "Type 'aws s3 get-policy shared-data-lake' to view.", intel: "Look for Principal: '*' or Principal: {'AWS': '*'} - both mean 'everyone in the world'.", completionFeedback: "You reviewed the policy. Reading policies before changing them helps you understand the blast radius." },
      { number: 3, title: "Analyze Permissions", description: "The policy grants 's3:*' to principal '*'. This allows anyone to read, write, and delete data.", hint: "Wildcard policies violate CIS Benchmark 2.1.5.", intel: "This is a common finding in penetration tests. Attackers use tools like 'bucket_finder' to discover misconfigured buckets.", completionFeedback: "You understand the risk. Principal '*' means anyone on the internet - a critical security gap." },
      { number: 4, title: "Apply Least Privilege", description: "Restrict the policy to specific actions and principals.", hint: "Type 'aws s3 restrict-policy shared-data-lake'.", intel: "Best practice: Use specific IAM roles instead of wildcards. Grant only read access if write isn't needed.", completionFeedback: "You applied least privilege. Restricting access to only what's needed is a core security principle." },
      { number: 5, title: "Verify New Policy", description: "Check that the policy is now restrictive.", hint: "Type 'aws s3 get-policy shared-data-lake' to confirm.", intel: "After fixing, test that legitimate users can still access what they need.", completionFeedback: "You verified the new policy. Always check your fix didn't break legitimate access." },
      { number: 6, title: "Final Verification", description: "Run security scan to confirm remediation.", hint: "Type 'scan' to verify.", intel: "Consider implementing S3 Block Public Access at the account level to prevent future misconfigurations.", completionFeedback: "Remediation confirmed! You closed a major security gap that attackers actively hunt for." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Detect Anomaly", description: "Scan for buckets with external access.", hint: "Type 'scan' to identify issues.", intel: "Cross-account access is a common lateral movement technique. Attackers grant their accounts access for persistent data theft.", completionFeedback: "You detected the anomaly. Identifying unauthorized access is the first step in incident response." },
      { number: 2, title: "List External Access", description: "Check which accounts have access to the bucket.", hint: "Type 'aws s3 check-access partner-data-exchange'.", intel: "MITRE ATT&CK T1537: Transfer Data to Cloud Account - Adversaries may exfiltrate data by granting themselves access.", completionFeedback: "You listed external accounts. Knowing who has access helps you spot unauthorized parties." },
      { number: 3, title: "Identify Unauthorized Accounts", description: "Account 999888777666 is not in our approved partners list. This could be a compromise.", hint: "Cross-reference with your organization's approved account list.", intel: "Pro tip: Maintain a documented list of approved external accounts. Audit this list quarterly.", completionFeedback: "You identified the rogue account. Cross-referencing against approved lists catches attackers hiding in plain sight." },
      { number: 4, title: "Review Bucket Policy", description: "Examine how the external access was granted.", hint: "Type 'aws s3 get-policy partner-data-exchange'.", intel: "Check CloudTrail for 'PutBucketPolicy' events to see WHO added this account and WHEN.", completionFeedback: "You reviewed the policy. Understanding how access was granted helps trace the attack chain." },
      { number: 5, title: "Revoke Unauthorized Access", description: "Remove access for unknown accounts.", hint: "Type 'aws s3 revoke-external partner-data-exchange'.", intel: "After revoking, monitor for re-addition attempts. If it happens again, you have an active compromise.", completionFeedback: "You revoked unauthorized access. Cutting off attacker access is critical for containment." },
      { number: 6, title: "Verify Remediation", description: "Confirm only authorized accounts remain.", hint: "Type 'aws s3 check-access partner-data-exchange' to verify.", intel: "Document this incident. If data was accessed, you may need to notify affected parties under GDPR/CCPA.", completionFeedback: "You verified remediation. Confirming the fix and documenting for compliance are essential final steps." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Identify Compliance Gap", description: "Scan for buckets missing WORM protection.", hint: "Type 'scan' to identify issues.", intel: "SEC Rule 17a-4 and FINRA Rule 4511 require broker-dealers to preserve records in non-rewriteable, non-erasable format.", completionFeedback: "You identified the compliance gap. WORM storage is legally required for financial and healthcare records." },
      { number: 2, title: "Check Object Lock Status", description: "Review current Object Lock configuration.", hint: "Type 'aws s3 check-object-lock compliance-audit-logs'.", intel: "Object Lock has two modes: Governance (admins can override) and Compliance (nobody can delete, not even root).", completionFeedback: "You checked Object Lock status. Understanding current state helps plan the remediation." },
      { number: 3, title: "Understand Requirements", description: "SOX and HIPAA require immutable audit logs. Without Object Lock, logs can be tampered with.", hint: "MITRE ATT&CK T1565: Data Manipulation.", intel: "Attackers who compromise systems often delete logs first to cover their tracks. Immutable logs are your insurance policy.", completionFeedback: "You understand why WORM matters. Attackers delete logs to hide - immutable storage preserves evidence." },
      { number: 4, title: "Enable Object Lock", description: "Configure WORM protection in compliance mode.", hint: "Type 'aws s3 enable-object-lock compliance-audit-logs'.", intel: "Warning: Compliance mode is PERMANENT. Objects cannot be deleted until retention expires. Choose retention period carefully.", completionFeedback: "WORM protection enabled! Your logs are now tamper-proof - even admins cannot delete them." },
      { number: 5, title: "Verify Protection", description: "Confirm Object Lock is active.", hint: "Type 'aws s3 check-object-lock compliance-audit-logs'.", intel: "Test by attempting to delete a locked object - you should receive an 'Access Denied' error even with admin permissions.", completionFeedback: "You verified Object Lock is active. Testing controls ensures they work when you need them." },
      { number: 6, title: "Generate Compliance Report", description: "Document the remediation for auditors.", hint: "Type 'scan' to generate report.", intel: "Keep screenshots and timestamps. Auditors love documentation that proves exactly when controls were implemented.", completionFeedback: "Compliance documented! Proof of remediation satisfies auditors and protects against regulatory fines." }
    ],
    resources: [
  ...
  // ADVANCED LABS (8-12 steps, complex scenarios)
  {
    ...
    steps: [
      { number: 1, title: "Initial Assessment", description: "Perform a comprehensive security scan of all S3 resources.", hint: "Type 'scan' to identify all vulnerabilities.", completionFeedback: "You performed a comprehensive audit. Seeing all issues at once helps prioritize remediation." },
      { number: 2, title: "Inventory Buckets", description: "List all buckets and understand their purpose.", hint: "Type 'aws s3 ls' to see all buckets.", completionFeedback: "You inventoried resources. Understanding purpose helps apply appropriate security controls." },
      { number: 3, title: "Check Block Public Access", description: "Verify Block Public Access settings on production bucket.", hint: "Type 'aws s3 check-block-public prod-application-data'.", completionFeedback: "You checked Block Public Access. This AWS feature prevents accidental public exposure." },
      { number: 4, title: "Enable Block Public Access", description: "Turn on all four Block Public Access settings.", hint: "Type 'aws s3 block-public-access prod-application-data'.", completionFeedback: "Block Public Access enabled! This overrides permissive bucket policies - a critical safeguard." },
      { number: 5, title: "Review Replication Config", description: "Check data replication compliance for EU data.", hint: "Type 'aws s3 check-replication eu-customer-data'.", completionFeedback: "You reviewed replication. Data residency is a GDPR requirement with heavy fines for violations." },
      { number: 6, title: "Fix Replication Region", description: "Data must stay in EU regions for GDPR. Update replication.", hint: "Type 'aws s3 fix-replication eu-customer-data'.", completionFeedback: "GDPR-compliant replication configured! EU data now stays in EU - avoiding regulatory penalties." },
      { number: 7, title: "Audit Pre-signed URLs", description: "Check pre-signed URL expiration policy.", hint: "Type 'aws s3 check-presigned temp-file-sharing'.", completionFeedback: "You audited URL policies. Long-lived pre-signed URLs are a data leak waiting to happen." },
      { number: 8, title: "Fix Pre-signed Policy", description: "Reduce URL expiration to limit exposure window.", hint: "Type 'aws s3 fix-presigned temp-file-sharing'.", completionFeedback: "Pre-signed policy hardened! Shorter expiration = smaller window for URL theft/misuse." },
      { number: 9, title: "Comprehensive Verification", description: "Run final security scan to confirm all issues resolved.", hint: "Type 'scan' to verify complete remediation.", completionFeedback: "You verified all fixes. Multi-resource hardening requires confirmation of each change." },
      { number: 10, title: "Document Findings", description: "All three vulnerabilities have been addressed: Block Public Access enabled, GDPR-compliant replication configured, and pre-signed URL policy hardened.", hint: "Mission complete when all resources show SECURE status.", completionFeedback: "Hardening complete! You secured three buckets with different risk profiles - real-world skills." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Initial Triage", description: "Security alerted on suspicious S3 access patterns. Begin investigation.", hint: "Type 'scan' to assess current security posture.", completionFeedback: "You started incident triage. Speed matters - attackers may still be accessing data." },
      { number: 2, title: "Review Bucket List", description: "Identify buckets containing sensitive data.", hint: "Type 'aws s3 ls' to list all buckets.", completionFeedback: "You identified affected resources. Knowing what's at risk guides response priority." },
      { number: 3, title: "Check Bucket Policy", description: "Examine the policy on the PII bucket.", hint: "Type 'aws s3 get-policy customer-pii-data'.", completionFeedback: "You examined the policy. Understanding the misconfiguration explains how the breach happened." },
      { number: 4, title: "Analyze Access Patterns", description: "The bucket was accessed from external IPs. The overly permissive policy allowed anonymous reads.", hint: "This is MITRE ATT&CK T1530: Data from Cloud Storage Object.", completionFeedback: "You analyzed the attack pattern. This matches MITRE ATT&CK T1530 - data theft from cloud storage." },
      { number: 5, title: "Check Encryption Status", description: "Verify if exposed data was encrypted.", hint: "Type 'aws s3 ls-encryption'.", completionFeedback: "You checked encryption. Encrypted data is less damaging if exfiltrated - it's unreadable." },
      { number: 6, title: "Secure the Bucket Policy", description: "Immediately restrict the bucket policy.", hint: "Type 'aws s3 restrict-policy customer-pii-data'.", completionFeedback: "Policy secured! You stopped the active data leak - containment is critical." },
      { number: 7, title: "Enable Encryption", description: "Encrypt the bucket to protect remaining data.", hint: "Type 'aws s3 enable-encryption customer-pii-data'.", completionFeedback: "Encryption enabled! Future data is protected even if accessed unauthorized." },
      { number: 8, title: "Enable Access Logging", description: "Turn on logging for future forensics.", hint: "Type 'aws s3 enable-logging customer-pii-data'.", completionFeedback: "Logging enabled! You'll now have evidence for any future suspicious activity." },
      { number: 9, title: "Block Public Access", description: "Prevent any future public exposure.", hint: "Type 'aws s3 block-public-access customer-pii-data'.", completionFeedback: "Public access blocked! This account-level control prevents future misconfigurations." },
      { number: 10, title: "Final Verification", description: "Confirm all security controls are in place.", hint: "Type 'scan' to verify remediation.", completionFeedback: "You verified all controls. Defense-in-depth is now active with multiple security layers." },
      { number: 11, title: "Incident Documentation", description: "Document the breach timeline: initial exposure, data accessed, remediation steps taken, and controls implemented.", hint: "All controls implemented. Incident contained.", completionFeedback: "Incident documented! This timeline is required for breach notifications and lessons learned." }
    ],
    resources: [
  ...
  },
  {
    ...
    steps: [
      { number: 1, title: "Threat Intelligence Alert", description: "TI team reports a supply chain attack. Malicious actors are compromising CI/CD artifacts.", hint: "Type 'scan' to identify vulnerable resources.", completionFeedback: "You received the threat intel. Acting on TI quickly can prevent compromise." },
      { number: 2, title: "List Deployment Buckets", description: "Identify all buckets in the deployment pipeline.", hint: "Type 'aws s3 ls' to review buckets.", completionFeedback: "You identified deployment assets. Supply chain attacks target CI/CD - knowing your surface is critical." },
      { number: 3, title: "Check Cross-Account Access", description: "Verify who has access to deployment artifacts.", hint: "Type 'aws s3 check-access deployment-artifacts'.", completionFeedback: "You audited access. Unauthorized accounts with write access can inject malicious code." },
      { number: 4, title: "Identify Unauthorized Access", description: "External account 999888777666 has write access - this is the attack vector.", hint: "MITRE ATT&CK T1195.002: Supply Chain Compromise.", completionFeedback: "You found the attack vector. This matches T1195.002 - supply chain compromise via artifacts." },
      { number: 5, title: "Review Bucket Policy", description: "Examine how the malicious access was granted.", hint: "Type 'aws s3 get-policy deployment-artifacts'.", completionFeedback: "You traced the compromise. Understanding HOW helps prevent future incidents." },
      { number: 6, title: "Revoke Malicious Access", description: "Immediately remove the unauthorized account.", hint: "Type 'aws s3 revoke-external deployment-artifacts'.", completionFeedback: "Malicious access revoked! You ejected the attacker from your build pipeline." },
      { number: 7, title: "Check Object Integrity", description: "Verify if Object Lock could prevent tampering.", hint: "Type 'aws s3 check-object-lock deployment-artifacts'.", completionFeedback: "You assessed integrity controls. Object Lock prevents tampering with signed artifacts." },
      { number: 8, title: "Enable Object Lock", description: "Prevent future artifact modification.", hint: "Type 'aws s3 enable-object-lock deployment-artifacts'.", completionFeedback: "Object Lock enabled! Your artifacts are now immutable - attackers can't modify them." },
      { number: 9, title: "Enable Versioning", description: "Allow rollback to known-good artifacts.", hint: "Type 'aws s3 enable-versioning deployment-artifacts'.", completionFeedback: "Versioning enabled! You can now rollback to pre-compromise artifacts if needed." },
      { number: 10, title: "Verify All Controls", description: "Confirm all security measures are in place.", hint: "Type 'scan' to verify.", completionFeedback: "You verified supply chain hardening. Multiple controls now protect your build pipeline." },
      { number: 11, title: "Post-Incident Actions", description: "All artifacts should be rescanned, deployment keys rotated, and build pipeline audited.", hint: "Supply chain secured. Recommend full artifact rescan.", completionFeedback: "Supply chain secured! You've completed a real-world response to T1195.002 compromise." }
    ],
    resources: [
